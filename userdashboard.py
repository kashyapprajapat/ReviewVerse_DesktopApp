...
#             print(f"Displaying user ID: {user_id}")
#             print(f"Displaying username: {username}")
...
class UserDashboard(QWidget):
    def __init__(self, username, user_id):
        ...
        # Username label at top-right
        self.username_label = QLabel(f"Welcome 🥳, {self.username}")
        self.username_label.setStyleSheet("font-size: 18px; font-weight: bold;")
        header_layout.addWidget(self.username_label, alignment=Qt.AlignRight)
    ...
    def open_write_review(self):
        ...
        """
        from writereview import WriteReview  # Import the WriteReview class
        self.write_review_window = WriteReview(self.user_id)  # Pass user_id to WriteReview
        self.write_review_window.show()  # Show the WriteReview window
    ...
    def fetch_reviews(self):
        ...
        """
        try:
            print(f"Displaying user ID: {self.user_id}")
            print(f"Displaying username: {self.username}")

            url = f"https://reviewverse.onrender.com/get-reviews/{self.user_id}"
            response = requests.get(url)
            data = response.json()
    ...
    def delete_review(self, row, review_id):
        ...
        if confirm == QMessageBox.Yes:
            try:
                url = f"https://reviewverse.onrender.com/delete-review/{self.user_id}/{review_id}"
                response = requests.delete(url)
